...
> _Note_: `KIN_AMAGAMBO.tandukanya()` returns an array (`ubwoko` is `urutonde`). You can index it with `parts[0]`, measure it with `KIN_URUTONDE.ingano(parts)`, and print it as `[a, b, c]`.
