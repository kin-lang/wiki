...
### Array Include (`ifite`) — first element only

`KIN_URUTONDE.ifite(list, value)` currently compares `value` to the **first element only**. It does not scan the rest of the list.
...
> First call prints `nibyo` because `"Hello"` is first.
> Second call prints `sibyo` today, even though `"World"` is in the list.
