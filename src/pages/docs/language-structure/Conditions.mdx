...
- Use `nanone_niba` to specify a **new** condition to test if the previous condition is false (else-if)
- Use `niba_byanze` to specify a block of code to be executed if every preceding condition is false (else — no condition)
